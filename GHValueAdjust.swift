...
public class GHValueAdjustView: UIView {
    ...
    public func setVolumeValue(_ percent: Int?) {
        let value = bounds.height*CGFloat(percent ?? 100)/100
        setThumbValue(value)
    }
    ...
    private func imageFromBundle(named name: String) -> UIImage {
        var bundle = Bundle(for: GHValueAdjustView.self)
        if let resourcePath = bundle.path(forResource: "GHValueAdjust", ofType: "bundle") {
